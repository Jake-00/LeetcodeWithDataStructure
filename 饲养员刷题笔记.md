...
|                              | Python3                                   | Java                                                         |
| ---------------------------- | ----------------------------------------- | ------------------------------------------------------------ |
| 1.创建堆（大顶堆 \| 小顶堆） | # 用 list 或者 deque 实现栈<br>stack = [] | import java.util.*;<br>Stack<Integer> st = new Stack<Integer>(); |
| 2.添加元素                   | stack.append(element)                     | st.push();                                                   |
| 3.获取堆顶的元素             | tmp = stack[-1]                           | st.peek();                                                   |
| 4.删除堆顶元素               | stack.pop()                               | st.pop();                                                    |
| 5.堆的长度                   | if len(stack)                             | st.empty()                                                   |
| 6.堆的遍历                   | while len(stack):<br/>    stack.pop()     | while (st.size()>0){<br/>    st.pop();<br/>}                 |
